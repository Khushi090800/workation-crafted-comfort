...
-- Drop the insecure SELECT policy on waitlist
DROP POLICY IF EXISTS "Admins can view waitlist" ON public.waitlist;

-- Create proper admin-only SELECT policy for waitlist
CREATE POLICY "Only admins can view waitlist"
ON public.waitlist
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- Add explicit DENY policies for UPDATE and DELETE on waitlist
